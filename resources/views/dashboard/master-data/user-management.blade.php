...
@php
    $sidebar = \App\Http\Controllers\Dashboard\MsUserManagementController::menu();
@endphp
                                                ...
                                                @foreach($s['menu'] as $m)
                                                    <div class="col-sm-4">
                                                        <div class="form-check">
                                                            ...
                                                            <label class="form-check-label" for="permission_{{ $m['id'] }}">{{ $m['name'] }}</label>
                                                        </div>
                                                    </div>
                                                @endforeach
                                            </div>
...
@section('script')
    <script>
        ...
        const tableIndex = $('#tableIndex').DataTable({
            "ajax": {
                "method": "POST",
                "url": "{{ url('admin/master-data/user-management/list') }}",
                "header": {
                    "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr('content'),
                },
        ...
        $(document).ready(function () {
            ...
            btnEdit.click(function (e) {
                ...
                $.ajax({
                    url: '{{ url('admin/master-data/user-management/user-permission') }}',
                    method: 'post',
            ...
            btnHapus.click(function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: '{{ url('admin/master-data/user-management/delete') }}',
                            method: 'post',
                            data: {username: iUsername.val()},
                            success: function (response) {
                                console.log(response);
            ...
            btnReset.click(function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: '{{ url('admin/master-data/user-management/reset') }}',
                            method: 'post',
                            data: {username: iUsername.val()},
                            success: function (response) {
                                console.log(response);
                                if (response === 'success') {
                                    Swal.fire({
                                        ...
                                        text: 'Username dan Password sama',
                                        type: 'success',
                                        onClose: function () {
                                            tableIndex.ajax.reload();
                                        }
            ...
            dataForm.submit(function (e) {
                ...
                if (inputType.val() === 'new') {
                    url = "{{ url('admin/master-data/user-management/add') }}";
                } else {
                    url = "{{ url('admin/master-data/user-management/edit') }}";
                }
                $.ajax({
                    url: url,
                    method: 'post',
